...
const BinaryRain: React.FC<BinaryRainProps> = ({ isActive = true }) => {
  ...
  const monitorPerformance = useCallback(() => {
    ...
    if (frameTimeRef.current.length > 120) {
      // Increased from 60 to 120 for more stable readings
      // Monitor last 120 frames
      frameTimeRef.current.shift()
    }

    if (frameTimeRef.current.length >= 30) {
      // Increased from 10 to 30 for more stable average
      const avgFrameTime =
      ...
      if (fps < 25) {
        // Lower threshold from 30
        newMode = 'low'
      } else if (fps < 40) {
        // Lower threshold from 45
        newMode = 'medium'
      } else if (fps > 50) {
        // Higher threshold to return to high
        newMode = 'high'
      }
    ...
    const createBinaryDigit = () => {
      ...
      const createMorphingContent = (pattern: string) => {
        ...
        const morphInterval = setInterval(
          ...
          Math.random() * 500 + 300,
        ) // Change character every 300-800ms
      ...
      const size = (() => {
        ...
        switch (pattern) {
          case 'matrix':
            return Math.random() * (baseSize[1] - baseSize[0]) + baseSize[0] + 2
          case 'glitch':
            return Math.random() * (baseSize[1] - baseSize[0]) + baseSize[0] + 4
          default:

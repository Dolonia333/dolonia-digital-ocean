...
export default function EngagementListener() {
  useEffect(() => {
    const handler = (event: Event) => {
      ...
      const actionable = (target.closest("[data-engagement]") ?? target) as HTMLElementWithEngagement | null;
      if (!actionable) {
        return;
      }
    ...
    EVENT_TYPES.forEach((eventType) => document.addEventListener(eventType, handler, { capture: true }));

    return () => {
      EVENT_TYPES.forEach((eventType) => document.removeEventListener(eventType, handler, { capture: true }));
    };
  }, []);
